...
foreach my $species (keys %species_to_analyze){
    ...
    run_exe("${opera_ms_dir}utils/perl $opera_ms_dir/bin/get_edges_from_matrix.pl $inter_dir $out_dir/$species/matrix $out_dir/$species $contigs_file 0 1000 2> $out_dir/$species/get_edge_matrix.err");
    if($?){
