...
sub generateAlignmentUsingBwa
{
    ...
    $command = "${path}bwa samse -n 1 $folder$contigName[ -1 ] ${folder}read.sai - | ${samtoolsDir}samtools view -S -h -b -F 0x4 - | ${samtoolsDir}samtools sort -\@ 20 -no - ${folder}temporarySam > $folder$outputFile";
    print $command."\n";
}
