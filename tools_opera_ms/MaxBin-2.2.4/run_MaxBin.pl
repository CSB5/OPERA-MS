...
my $BOWTIE2BUILD = "/mnt/projects/bertrandd/opera_lg/SOFTWARE/MAXBIN/MaxBin-2.2.4/auxiliary/bowtie2-2.2.3/bowtie2-build";
my $BOWTIE2 = "/mnt/projects/bertrandd/opera_lg/SOFTWARE/MAXBIN/MaxBin-2.2.4/auxiliary/bowtie2-2.2.3/bowtie2";
my $HMMSEARCH = "/mnt/projects/bertrandd/opera_lg/SOFTWARE/MAXBIN/MaxBin-2.2.4/auxiliary/hmmer-3.1b1/src/hmmsearch";
my $RUNFRAG = "/mnt/projects/bertrandd/opera_lg/SOFTWARE/MAXBIN/MaxBin-2.2.4/auxiliary/FragGeneScan1.30/run_FragGeneScan.pl";
my $IDBA_UD = "/mnt/projects/bertrandd/opera_lg/SOFTWARE/MAXBIN/MaxBin-2.2.4/auxiliary/idba-1.1.1/bin/idba_ud";
...
sub main
{
	...
	while ($currnum < $maxnum)
	{
	    ...
	    print STDERR " **** Search seed for $currbin $currnum $contig_f.hmmout\n";
...
sub runBowtie2
{
	...
	if ($isfastq eq "fq" || $isfastq eq "fqgz")
	{
	    ...
	    $cmd = "$BOWTIE2 -p $THREADNUM -x $outheader.idx -U $reads_f | samtools view -bSF4 - 1> ${out_f}.bam 2>$out_f.bowtie2.err";
    }
	elsif ($isfastq eq "fa" || $isfastq eq "fagz")
	{
	    ...
	    $cmd = "$BOWTIE2 -f -p $THREADNUM -x $outheader.idx -U $reads_f | samtools view -bSF4 - 1>${out_f}.bam 2>$out_f.bowtie2.err";
    }
	system($cmd);
	if (checkResult("${out_f}.bam") == -1)
	{
