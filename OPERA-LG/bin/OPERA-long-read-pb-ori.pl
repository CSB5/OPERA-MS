...
sub printEdges {
	...
	for( $j = $i + 1; $j <= $alignemnt_to_link; $j++ ){
	    ...
	    #Update the structure where we store the distance and the number of edges that support
	    $edges{$edge_ID}->[0]++;
...
sub checkMapping{
    ...
    my @edge_distance_support = ();my @sorted_distance = ();
    my ($distance_ID, $best_support);
    foreach $key (keys(%edge_read_info)) {
	print STDERR " *** contig pair $key\n";
	#Compute the support and edge distance
	@edge_distance_support = ();
	@sorted_distance = (sort {$b <=> $a} @{$edges{$key}}[1..@{$edges{$key}}-1]);
	for(my $i = 0; $i < @sorted_distance; $i++){
	    $distance = $sorted_distance[$i];
	    print STDERR " *** the dist $distance\n";
	    $distance_ID = -1;
	    for(my $j = 0; $j < @edge_distance_support; $j++){
		#The distance match one the the previous distance found
		if(remove_standard_deviation($edge_distance_support[$j]->[0]) <= $distance && $distance <= add_standard_deviation($edge_distance_support[$j]->[0])){
		    $distance_ID = $j;
		    last;
		}
	    }
		...
		push(@edge_distance_support, [$distance, $distance, 1]);
	    }
	    else{
		#Update the edge distance and the support if the distance are similar
		$edge_distance_support[$distance_ID]->[1] += $distance;
		$edge_distance_support[$distance_ID]->[2]++;
	    }
	}
	
	print STDERR " *** ".(@edge_distance_support)." number of discordant distances\n";

	#Compute the final support
	$distance_ID = 0;
	$best_support = $edge_distance_support[0]->[2];
	for(my $j = 1; $j < @edge_distance_support; $j++){
	    $support = $edge_distance_support[$j]->[2];
	    if($best_support < $support){
		$best_support = $support;
		$distance_ID = $j;
	    }
	}
	$distance = $edge_distance_support[$distance_ID]->[1] / $edge_distance_support[$distance_ID]->[2];#Compute the average distance 
	#$edges{$key}->[0] = $distance;#The final distance for that edge use to filter the read info
	...
	#$nb_read_in_edge = @{$edge_read_info{$key}->["DISTANCE"]};
	$splice_offset = 0;
	#while($cmp_read < $nb_read_in_edge){
	for(my $j = 0; $j < @{$edge_read_info{$key}->{"DIST_LIST"}}; $j++){
	    $read_distance = $edge_read_info{$key}->{"DIST_LIST"}->[$j];#cmp_read];
	    if(! (remove_standard_deviation($distance) <= $read_distance && $read_distance <= add_standard_deviation($distance))){#this read do not have the correct distance
		print STDERR " *** Remove reads ".($edge_read_info{$key}->{"READ_LIST"}->[$splice_offset])." $read_distance not in [ ".(remove_standard_deviation($distance))." ,".(add_standard_deviation($distance))."] from $distance and supported by $best_support reads n";
		#splice @{$edge_read_info{$key}->["DISTANCE"]}, $cmp_read, 1;
		...
		splice @{$edge_read_info{$key}->{"CONTIG_FOR_GAPFILLING"}}, $splice_offset, 1;
		#$nb_read_in_edge--;
	    }
	    else{
	    	$splice_offset++;
