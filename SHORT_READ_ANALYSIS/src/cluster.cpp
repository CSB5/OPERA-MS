...
Cluster::Cluster(sContig* contig) {
	num_contigs_ = 1;
	contigs_ = new sContig*[num_contigs_];
...
sContig** Cluster::contigs() const { return contigs_; }
void Cluster::set_contigs(sContig** contigs) { contigs_ = contigs; }
void Cluster::set_contigs(std::vector<sContig*> *v_contig){
	contigs_ = new sContig*[v_contig->size()];
	int cmp = 0;
