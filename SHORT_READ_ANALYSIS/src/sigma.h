...
public:
	...
	/* main constructor !!! */
	/**
	 * @brief Constructs sigma main function.
	 *
	 * @param argc		argc
	 * @param argv          argv
	 */
	Sigma(char* configFile);
	void init();

	//To update the contig coverage while reading sam file
	void set_contig_coverage(const char* qname, int flag, const char* contig_id, int read_pos, int read_length);
	void save_contigs();
	//
	~Sigma();
