...
int main(int argc, char *argv[] )
{
	...
	char* sigmaConfigFileName = argv[ 2 ];
	if( argc == 1 || argc > 5 || string(argv[ 1 ]) == "-h" ){
		cout<<"Usage:\n";
	...
	//For the sigma
	Sigma *m_sigma = new Sigma(sigmaConfigFileName);
	...
	if( myContigConverter.ConvertContigFile( Configure::CONTIG_FILE, m_opera->m_graph, m_opera->m_libraries, m_sigma ) == -1 ){
		cout<<"ERROR: Converting contig file error!"<<endl;
		return -1;
	}
	//Write the contig coverage file
	m_sigma->save_contigs();
	//exit(1);
