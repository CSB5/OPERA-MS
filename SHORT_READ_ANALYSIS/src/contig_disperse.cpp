...
sContig::sContig(std::string id, int length) : id_(id), length_(length) {
...
sContig::sContig(std::string id, int length, int left_edge, int right_edge, int num_windows) :
...
std::string sContig::id() const { return id_; }
int sContig::length() const { return length_; }

int sContig::modified_length() const { return modified_length_; }
int sContig::left_edge() const { return left_edge_; }
int sContig::right_edge() const { return right_edge_; }
int sContig::num_windows() const { return num_windows_; }

int* sContig::sum_read_counts() const { return sum_read_counts_; }
int** sContig::read_counts() const { return read_counts_; }

Cluster* sContig::cluster() const { return cluster_; }

void sContig::set_cluster(Cluster* cluster) { cluster_ = cluster; }
...
void ContigIO::load_contigs(const char* sigma_contigs_file_path, ContigMap* contigs) {
	...
	if (sigma_contigs_fp != NULL) {
		...
		while (!feof(sigma_contigs_fp)) {
			...
			sContig* contig = new sContig(id, length, left_edge, right_edge, num_windows);
