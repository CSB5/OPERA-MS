...
// Convert contig file
// return: -1 if failed
int ContigConverter::ConvertContigFile( string fileName, Graph *graph, list<PetLibrary*> *libs, Sigma *m_sigma  ){
	...
	if( Configure::FILE_FORMAT != STATISTIC ){
		if( CalculateCovUsingMapping( libs, m_sigma ) == -1 )
			return -1;
	}
...
int ContigConverter::CalculateCovUsingMapping( list<PetLibrary*> *libs, Sigma *m_sigma ){

	//cerr << "MAP TYPE : "<< (Configure::MAP_TYPE) << endl;
	...
	for( int libID = 0; libID < (int) Configure::MULTI_LIB_INFO->size(); libID++ ){
		if(Configure::MULTI_LIB_INFO->at( libID )-> GetMapType() == SAM || Configure::MULTI_LIB_INFO->at( libID )-> GetMapType() == BOWTIE){
			...
			//Hard coded to only str
			//ifstream mapReader( tempFileName.c_str() );
			istream* mapReader = &cin;		
			///////
			...
			while( getline( *mapReader, nextLine ) ){
				try{     
					...
					//Update sigma coverage
					//char* qname, int flag, char* contig_id, int read_pos, int read_length => read from sam file
					m_sigma->set_contig_coverage((*preColumn)[0].c_str(), atoi((*preColumn)[1].c_str()), (*preColumn)[2].c_str(), atoi((*preColumn)[3].c_str()), (*preColumn)[9].length());
					...
					if( IsPair( preNameString, nextNameString ) ){
						...
						if( (*preColumn)[ 2 ] == (*nextColumn)[ 2 ] )
							...
							if( firstContig != NULL && secondContig != NULL ){
								//cerr << "|" << firstContig << "|" << firstContigPos << "|" << firstContigOri << "|" << firstReadLength << "|" << endl;
