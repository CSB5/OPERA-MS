...
void Sigma::configure(ParamsMap* params) {
	...
	R_ESTIMATION_TYPE = getIntValue(params, std::string("R_ESTIMATION_TYPE"));
	...
	if(R_ESTIMATION_TYPE == -1) R_ESTIMATION_TYPE = 1;
...
int main(int argc, char** argv) {
	...
	fprintf(stderr, "Toal number of contigs: %ld\n\n", Sigma::total_assembly_nb_contig);
	...
	if (Sigma::pdist_type == "Poisson") {
		...
		if (Sigma::R <= 0.0) {
			Sigma::R = compute_R(&contigs);
			//prob_dist = new NegativeBinomialDistribution(compute_R(&contigs));
		} 
		prob_dist = new NegativeBinomialDistribution(Sigma::R);
