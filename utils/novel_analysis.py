...
def get_sketch(query_files, sketch_size, kmer_size, outdir, file_type, sketch_name): 
    ...
    run_exe(cmd)    
    cmd = "{}/mash sketch -k {} -s {} -o {} {}  > {}/{}.out 2> {}/{}.err ".format(script_dir, kmer_size, sketch_size, sketch, query_files, outdir, sketch_name, outdir, sketch_name)
    run_exe(cmd)
    return sketch+".msh"
...
def get_cluster(script_dir, hclust_thres, dist_file, outdir, file_type):
    ...
    try:
        ...
        pass
    cmd = "Rscript --vanilla {}/clustering.r {} {} {}".format(script_dir, dist_matrix,  hclust_thres, cluster_file)
    run_exe(cmd)
...
def main(args):
    ...
    try:
        ...
        pass    
    cmd = "{}/mash dist -p 8 -d 0.85 {} {} >> {}".format(script_dir, ref_sketch, query_sketch,  query_ref_dist_file)
    run_exe(cmd)

    novel_sequence, novel_seq_files = get_novel_sequence(query_ref_dist_file, query_ref_outdir)
    #novel_sequence, novel_seq_files = get_novel_sequence(query_ref_dist_file, query_ref_outdir,  genome_hash)# add genome that do not have any distance
    ...
    try:
        os.remove(novel_dist_file)
    except:
        pass
    cmd = "{}/mash dist -d 0.50 {} {} >> {}".format(script_dir, novel_sketch, novel_sketch, novel_dist_file)
    run_exe(cmd)
    #generate matrix
